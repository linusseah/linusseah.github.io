...
The results below are a snippet of the original output after converting word features into numerical form using the CountVectorizer. We are clearly facing a **sparse-matrix problem**. A sparse-matrix occurs when there are many unique (word) features that occur only once or a few times across in the entire dataset, leaving the matrix with many null values, and hence resulting in a 'sparse-matrix'. This is problematic because we now have an extremely tall (high number of rows) and wide (high number of columns) dataset that makes it computationally expensive to be trained on. 
...
<figure>
	...
	<figcaption> Count of all unique word features </figcaption>
</figure>
